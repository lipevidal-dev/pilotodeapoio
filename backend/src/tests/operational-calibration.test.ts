...
describe("Calibração operacional — regra folgas 10/11", () => {
  ...
  it("14. motor prioriza 10 folgas quando possível", () => {
    const input = realisticGenerationInput();
    const result = engine.generate(input);
    const counts = input.employees
      .filter((e) => e.employee.role === "PAO")
      .map((e) => result.allocations.filter((a) => a.employeeUuid === e.uuid && a.label.includes("FOLGA")).length);
    const with11 = counts.filter((n) => n === MAX_PAO_REST_COUNT).length;
    const with10 = counts.filter((n) => n === IDEAL_PAO_REST_COUNT).length;
    expect(with10 + with11).toBe(input.employees.filter((e) => e.employee.role === "PAO").length);
    expect(with11).toBeLessThanOrEqual(with10);
  }, SLOW_MS);
});
...
describe("Calibração operacional — T8 e fechamento", () => {
  ...
  it("16b. blocos de 3+ folgas priorizados", () => {
    const input = realisticGenerationInput();
    const result = engine.generate(input);
    const days = Array.from({ length: 30 }, (_, i) => `2026-06-${String(i + 1).padStart(2, "0")}`);
    const restLabels = new Set(["FOLGA", "FOLGA SOCIAL", "FOLGA AGRUPADA", "FOLGA PEDIDA", "FOLGA ESCOLHIDA"]);
    let blocks3Plus = 0;
    for (const e of input.employees.filter((x) => x.employee.role === "PAO")) {
      const restDays = result.allocations
        .filter((a) => a.employeeUuid === e.uuid && restLabels.has(a.label.toUpperCase()))
        .map((a) => days.indexOf(a.date))
        .filter((i) => i >= 0)
        .sort((a, b) => a - b);
      for (let i = 0; i < restDays.length; ) {
        let run = 1;
        while (i + run < restDays.length && restDays[i + run] === restDays[i] + run) run++;
        if (run >= 3) blocks3Plus++;
        i += run;
      }
    }
    expect(blocks3Plus).toBeGreaterThan(0);
  }, SLOW_MS);
