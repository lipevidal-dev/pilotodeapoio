...
describe("completePaoAgenda", () => {
  ...
  it("9. REAL_V1 gera turnos + FS; folga comum manual no cenário-base", () => {
    const input = realisticGenerationInput();
    const result = engine.generate(input);
    ...
    expect(result.assignments.filter((a) => ["T6", "T7", "T8"].includes(a.shiftCode)).length).toBeGreaterThan(0);
  }, SLOW_MS);
