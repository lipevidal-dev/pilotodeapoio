...
describe("Fase 8.0 — Motor real v1", () => {
  ...
  it("16. REAL_V1 não gera folga comum automaticamente (folga manual)", () => {
    ...
    const autoCommon = result.allocations.filter(
      (a) =>
        a.label === "FOLGA" &&
        result.summary.operationalByEmployee?.some(
          (e) => e.employeeUuid === a.employeeUuid && e.type === "PAO",
        ),
    );
  ...
  it("20. Botão Gerar Escala usa V5 por padrão; V4 via RealScheduleEngine", () => {
    const result = new ScheduleGenerationEngine().generate(minimalPaoInput(3));
    expect(result.summary.motorVersion).toBe("REAL_V5");
    expect(result.summary.realEngineExecuted).toBe(true);
    const v4 = realScheduleEngine.generate(minimalPaoInput(3));
    expect(v4.summary.motorVersion).toBe(MOTOR_VERSION_ID);
    expect(v4.suggestions.some((s) => s.includes("Motor real v1"))).toBe(true);
...
describe("REAL_V1 — regras estruturais críticas", () => {
  ...
  it("Gerar Escala V4 (RealScheduleEngine) retorna REAL_V1 com pipeline estrutural", () => {
    const result = realScheduleEngine.generate(realisticGenerationInput());
    expect(result.summary.motorVersion).toBe(MOTOR_VERSION_ID);
    expect(result.summary.realEngineExecuted).toBe(true);
    const report = result.summary.realMotorReport as { t8IsolatedCount?: number };
