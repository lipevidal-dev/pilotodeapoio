...
import { allocateOpportunitiesBelowMinimum } from "../domain/schedule/v6-opportunity-allocation.js";
...
describe("v6-opportunity-allocation", () => {
  it("tenta alocar PAO abaixo do mínimo em dia vazio (senioridade primeiro)", () => {
    const ws = new GenerationWorkspace(julyInput([pao(1, 1), pao(2, 2)]));
    ws.applyHardBlocks();
    ws.initRateioContext();
    const ctx = ws.ensureRateioContext();

    const report = allocateOpportunitiesBelowMinimum(ws, []);
    ws.syncRateioContext();

    expect(report.attempts).toBeGreaterThan(0);
    expect(currentTurnCount(ctx, "pao-1")).toBeGreaterThan(0);
    expect(ws.v55MinimumOpportunityAudit.some((r) => r.accepted > 0)).toBe(true);
  });
});
