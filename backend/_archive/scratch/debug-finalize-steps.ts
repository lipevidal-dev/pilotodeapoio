...
function step(name: string, fn: () => void) {
  ...
  console.log(name, "T8-21", scratch.findPaoOnShift("2026-06-21", "T8"), "gaps", scratch.listCoverageGaps().length);
}

step("repairIsolatedT8", () => scratch.repairIsolatedT8());
step("reconcileNdAfterParallelShifts", () => scratch.reconcileNdAfterParallelShifts());
step("ensureNdForT8Pairs", () => scratch.ensureNdForT8Pairs());
step("cleanupOrphanNd", () => scratch.cleanupOrphanNd());
